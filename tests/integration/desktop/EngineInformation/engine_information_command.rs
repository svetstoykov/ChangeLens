...
use changelens_desktop_lib::engine_information::{
    EngineCommandError, EngineInformation, EngineInformationService, EngineState,
};
use std::sync::Arc;
use tauri::test::{INVOKE_KEY, get_ipc_response, mock_builder, mock_context, noop_assets};

struct FixedEngineInformationService {
    result: Result<EngineInformation, EngineCommandError>,
}
...
fn serializes_registered_engine_information_command_error() {
    let response = invoke_engine_information(Err(EngineCommandError::new(
        "engine.fixtureUnavailable",
        "The controlled engine fixture is unavailable.",
    )))
    ...
    assert_eq!(
        response,
        serde_json::json!({
            "code": "engine.fixtureUnavailable",
            "message": "The controlled engine fixture is unavailable.",
        })
    );
}
