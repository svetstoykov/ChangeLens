...
public sealed class EvidenceBinderServiceTests
{
    ...
    public void BinderIncludesRenameProvenanceAndOmitsEdgesAndExcludedContentFromPayload()
    {
        ...
        var result = new EvidenceBinderService(new EvidenceBinderOptions { MaximumDeveloperContextCharacters = 12 }).Assemble(
    ...
    public void PolicyInvariantDropsAreDiagnosedSeparatelyFromPolicyExclusions()
    {
        ...
        var result = new EvidenceBinderService(new EvidenceBinderOptions()).Assemble(request, CancellationToken.None);
    ...
    public void BudgetLadderDropsInDefinedOrderAndRecordsNodeIds()
    {
        ...
        var result = new EvidenceBinderService(new EvidenceBinderOptions { MaximumBinderCharacters = 6_000, TargetUtilization = 1 }).Assemble(
        ...
        Assert.DoesNotContain(binder.ChangedFiles, file => file.Path == "candidate.cs");
        Assert.Single(binder.ChangedFiles);
    }
    ...
    public void IrreducibleMandatoryContentFailsTheHardBudget()
    {
        ...
        var result = new EvidenceBinderService(new EvidenceBinderOptions { MaximumBinderCharacters = 1, TargetUtilization = 1 }).Assemble(
            request, CancellationToken.None);
